 ...
 * Option premium SL/TP on that strike (default −5 / +5) · day +5/−10 new entries · 1 open.
...
function normalizeSettings(raw = {}) {
  ...
  s.optionSlPts = Math.max(
    1,
    Number(s.optionSlPts) || (Number.isFinite(legacySl) ? legacySl : 5) || 5,
  );
  s.optionTpPts = Math.max(
    1,
    Number(s.optionTpPts) || (Number.isFinite(legacyTp) ? legacyTp : 5) || 5,
  );
