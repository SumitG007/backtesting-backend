/**
 * OI Cover Flip (OCF) — paper live.
 * Cover UP → ATM CE · +target/−stop · harvest until SL after target · 1 recovery · IST window.
 */
const LivePaperTrade = require('../models/livePaperTrade');
const LiveWallet = require('../models/liveWallet');
const { OI_COVER_FLIP_LIVE_KEY } = require('../strategies/keys');
const { buildSignalFromOiFlow } = require('../strategies/oiCoverFlip/signals');
...
const STRATEGY_KEY = OI_COVER_FLIP_LIVE_KEY;
const WALLET_KEY = 'paper_live_oi_cover_flip';
const STRATEGY_ID = 'oi-cover-flip';
...
async function ensureWallet() {
  ...
  if (!wallet) {
    wallet = await LiveWallet.create({
      ...
      realizedPnl: 0,
      cashLedger: false,
      oiCoverFlipEngineSettings: { ...DEFAULT_SETTINGS },
    });
  }
  return wallet;
}
...
async function loadSettingsFromDb() {
  ...
  engineState.settings = normalizeSettings(wallet.oiCoverFlipEngineSettings || {});
...
async function saveSettingsToDb(partial = {}) {
  ...
  const next = normalizeSettings({
    ...(wallet.oiCoverFlipEngineSettings?.toObject?.() || wallet.oiCoverFlipEngineSettings || {}),
    ...partial,
  });
  wallet.oiCoverFlipEngineSettings = next;
  await wallet.save();
...
async function refreshDayBook(dateKey) {
  if (engineState.dayPtsDateKey !== dateKey) {
    ...
    engineState.hadTargetToday = false;
    engineState.slWithoutTarget = 0;
  ...
  for (const t of closed) {
    const reason = String(t.reason || '').toUpperCase();
    if (reason === 'TARGET') hadTarget = true;
    if (reason === 'STOP_LOSS') {
      if (hadTarget) {
        dayDone = true;
        dayLockReason = 'SL after harvest · day done';
      } else {
        slNoTarget += 1;
        if (slNoTarget >= 2) {
          dayDone = true;
          dayLockReason = '2nd SL · day done';
        }
      }
    }
  }
  engineState.hadTargetToday = hadTarget;
  engineState.slWithoutTarget = slNoTarget;
...
async function tryEnter(signal, tape) {
  ...
  // No daily trade cap / day lock for OCF.
  ...
  try {
    ...
    const tradeDoc = await LivePaperTrade.create({
      ...
      entryReason: `OI Cover Flip ${optionType} · ${signal.barTime || ''} · ${signal.patternName || ''}`,
      notes: `oi_cover_flip; pattern=${signal.patternId}; niftySL=${stopSpot}; niftyTP=${targetSpot}; R=${riskPts}; recipe=${signal.recipeKey}; entrySrc=${entrySource}`,
      signalSnapshot: {
...
async function ensureEngineRunning() {
  if (!engineState.running) {
    ...
    startLoop();
    tickOnce().catch(() => {});
    console.log('OI Cover Flip (OCF) paper engine started');
...
async function closeOpenTradeManual(reason = 'MANUAL_CLOSE') {
  ...
  if (!open) throw new Error('No open OI Cover Flip trade');
  const mark = await resolveOptionLtp(open);
