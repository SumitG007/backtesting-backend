/**
 * OI Trap Expansion (OTE) — paper live.
...
const { OI_TRAP_EXPANSION_LIVE_KEY } = require('../strategies/keys');
const { buildSignalFromOiFlow } = require('../strategies/oiTrapExpansion/signals');
...
const STRATEGY_KEY = OI_TRAP_EXPANSION_LIVE_KEY;
const WALLET_KEY = 'paper_live_oi_trap_expansion';
const STRATEGY_ID = 'oi-trap-expansion';
...
async function ensureWallet() {
  ...
  if (!wallet) {
    wallet = await LiveWallet.create({
      ...
      realizedPnl: 0,
      cashLedger: false,
      oiTrapExpansionEngineSettings: { ...DEFAULT_SETTINGS },
    });
  }
  return wallet;
}
...
function normalizeSettings(raw = {}) {
  ...
  s.stopPts = Math.max(0.5, Math.min(100, Number(s.stopPts) || 8));
...
async function loadSettingsFromDb() {
  ...
  engineState.settings = normalizeSettings(wallet.oiTrapExpansionEngineSettings || {});
...
async function saveSettingsToDb(partial = {}) {
  ...
  const next = normalizeSettings({
    ...(wallet.oiTrapExpansionEngineSettings?.toObject?.() || wallet.oiTrapExpansionEngineSettings || {}),
    ...partial,
  });
  wallet.oiTrapExpansionEngineSettings = next;
  await wallet.save();
...
async function refreshDayBook(dateKey) {
  if (engineState.dayPtsDateKey !== dateKey) {
    ...
    engineState.hadProfitToday = false;
    engineState.slAfterProfit = 0;
  ...
  for (const t of closed) {
    const pts = favorPtsFromTrade(t);
    const reason = String(t.reason || '').toUpperCase();
    if (pts > 0 || reason === 'TARGET' || reason === 'EOD' || reason === 'DAY_CLOSE' || reason === 'FLIP_OPPOSITE') {
      if (pts > 0) hadProfit = true;
    }
    if (reason === 'STOP_LOSS') {
      if (hadProfit) {
        slAfter += 1;
        dayDone = true;
        dayLockReason = '1 SL after green · day done';
      }
    } else if (pts > 0) {
      hadProfit = true;
    }
  }
  // recompute properly chronologically
  hadProfit = false;
  slAfter = 0;
  dayDone = false;
  dayLockReason = null;
  ...
  engineState.hadProfitToday = hadProfit;
  engineState.slAfterProfit = slAfter;
...
async function checkOpenTrade(signal, tape) {
  ...
  // Opposite Cover Flip signal → close for reverse
  if (false && signal?.flipOpposite && signal?.status === 'TAKE_ENTRY') {
    ...
    if (Number.isFinite(favor) && favor > 0) engineState.hadProfitToday = true;
    return;
  }

  if (Number.isFinite(spotNow) && spotNow > 0 && Number.isFinite(stopSpot)) {
    const hit = isCall ? spotNow <= stopSpot : spotNow >= stopSpot;
    if (hit) {
      await finalizeTrade(open, {
        exitPremium: mark.optionLtp,
        mark,
      ...
      if (engineState.hadProfitToday) {
        engineState.dayDone = true;
        engineState.dayLockReason = '1 SL after green · day done';
      }
      return;
    }
  }
...
async function tryEnter(signal, tape) {
  ...
  // No daily trade cap / day lock for OTE.
  ...
  try {
    ...
    const tradeDoc = await LivePaperTrade.create({
      ...
      entryReason: `OI Trap Expansion ${optionType} · ${signal.barTime || ''} · ${signal.patternName || ''}`,
      notes: `oi_trap_expansion; pattern=${signal.patternId}; niftySL=${stopSpot}; niftyTP=${targetSpot}; R=${riskPts}; recipe=${signal.recipeKey}; entrySrc=${entrySource}`,
      signalSnapshot: {
...
async function tickOnce() {
  ...
  try {
    ...
    const allowAfterFlip = false && signal?.flipOpposite && closedThisTick;

    if (!engineState.openTradeId && (!closedThisTick || allowAfterFlip)) {
...
async function ensureEngineRunning() {
  if (!engineState.running) {
    ...
    startLoop();
    tickOnce().catch(() => {});
    console.log('OI Trap Expansion (OTE) paper engine started');
...
async function closeOpenTradeManual(reason = 'MANUAL_CLOSE') {
  ...
  if (!open) throw new Error('No open OI Trap Expansion trade');
  const mark = await resolveOptionLtp(open);
