...
async function checkOpenTrade(signal, tape) {
  ...
  if (signal?.flipOpposite && signal?.status === 'TAKE_ENTRY') {
...
async function tryEnter(signal, tape) {
  ...
  // No daily trade cap / day lock for OCC.
  ...
  try {
    ...
    const tradeDoc = await LivePaperTrade.create({
      ...
      entryReason: `OI Cover Chase ${optionType} · ${signal.barTime || ''} · ${signal.patternName || ''}`,
      notes: `oi_cover_chase; pattern=${signal.patternId}; niftySL=${stopSpot}; niftyTP=${targetSpot}; R=${riskPts}; recipe=${signal.recipeKey}; entrySrc=${entrySource}`,
      signalSnapshot: {
...
async function tickOnce() {
  ...
  try {
    ...
    const allowAfterFlip = signal?.flipOpposite && closedThisTick;
...
async function ensureEngineRunning() {
  if (!engineState.running) {
    ...
    startLoop();
    tickOnce().catch(() => {});
    console.log('OI Cover Chase (OCC) paper engine started');
...
async function closeOpenTradeManual(reason = 'MANUAL_CLOSE') {
  ...
  if (!open) throw new Error('No open OI Cover Chase trade');
  const mark = await resolveOptionLtp(open);
