/**
 * Flow Match Scalp — paper live.
 * Closed 3m Strong Bull/Bear + Match + spot with flow · green strike candle ·
 * +2/−3 · 09:30–14:30 · day ₹2k @ 10 lots · 15m after SL · 1 open · EOD 15:15.
 */
...
const STRATEGY_KEY = FLOW_MATCH_SCALP_LIVE_KEY;
const WALLET_KEY = 'paper_live_flow_match_scalp';
const STRATEGY_ID = 'flow-match-scalp';
...
async function ensureWallet() {
  ...
  if (!wallet) {
    wallet = await LiveWallet.create({
      ...
      realizedPnl: 0,
      cashLedger: false,
      flowMatchScalpEngineSettings: { ...DEFAULT_SETTINGS },
    });
  }
  return wallet;
}
...
async function loadSettingsFromDb() {
  ...
  engineState.settings = normalizeSettings(wallet.flowMatchScalpEngineSettings || {});
...
async function saveSettingsToDb(partial = {}) {
  ...
  const next = normalizeSettings({
    ...(wallet.flowMatchScalpEngineSettings?.toObject?.() || wallet.flowMatchScalpEngineSettings || {}),
    ...partial,
  });
  wallet.flowMatchScalpEngineSettings = next;
  await wallet.save();
...
async function tryEnter(signal, tape) {
  ...
  const barMin = Number(signal.barMinutes);
  const lastBar = Number(engineState.lastEntryBarMinutes);
  if (Number.isFinite(barMin) && Number.isFinite(lastBar) && barMin === lastBar) {
    engineState.lastEntryDebug = { skip: 'same_3m_bar', barMinutes: barMin };
    return;
  }
...
async function ensureEngineRunning() {
  if (!engineState.running) {
    ...
    startLoop();
    tickOnce().catch(() => {});
    console.log('Flow Match Scalp paper engine started');
