...
function computeMedianOrIv(orIvByDay, sortedKeys, dayIndex, ivLookbackDays) {
  const histOr = [];
  ...
  if (histOr.length < 5) return { medianOrIv: null, sampleSize: histOr.length };
  return { medianOrIv: median(histOr), sampleSize: histOr.length };
}

function evaluateIvSpikeSignal({ todayOr, medianOrIv, ivSpikeMultiplier, maxSpikeMultiplier }) {
  if (!todayOr || todayOr <= 0 || !medianOrIv || medianOrIv <= 0) {
    return { ok: false, reason: 'INSUFFICIENT_OR_DATA' };
  }
  const spike = Math.max(1.05, Number(ivSpikeMultiplier) || 1.25);
  const maxSpike = Math.max(spike + 0.1, Number(maxSpikeMultiplier) || 2);
  if (todayOr < medianOrIv * spike) {
  ...
  return { ok: true, reason: 'SPIKE_OK', todayOr, medianOrIv, spike, maxSpike };
}

function isInEntryWindow(minutes) {
  return minutes >= M1000 && minutes <= M1100;
}
...
function normalizeIvSettings(raw = {}) {
  ...
  return {
    ...
    ivSpikeMultiplier: Math.max(1.05, Number(raw.ivSpikeMultiplier) || 1.25),
    maxSpikeMultiplier: Math.max(
      1.15,
      Math.max(Number(raw.ivSpikeMultiplier) || 1.25, Number(raw.maxSpikeMultiplier) || 2),
    ),
