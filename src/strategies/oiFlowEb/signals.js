 ...
 * Risk is fixed option-premium SL/TP from settings (default 5 / 5). Spot is entry trigger only.
 */
const {
  build5mBars,
  matchLivePattern,
...
function buildSignalFromOiFlow(tape, settings = {}, opts = {}) {
  ...
  const optionSlPts = Math.max(
    1,
    Number(settings.optionSlPts) || Number(settings.riskMax) || 5,
  );
  const optionTpPts = Math.max(
    1,
    Number(settings.optionTpPts) || Number(settings.tpCap) || 5,
  );
