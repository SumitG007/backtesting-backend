...
function runStrategyThree({ candles, settings }) {
  const symbol = String(settings.symbol || 'BANKNIFTY').toUpperCase();
  const qty = Number(settings.qty);
  const fastPeriod = Math.max(2, Number(settings.fastMaPeriod));
  const slowPeriod = Math.max(fastPeriod + 1, Number(settings.slowMaPeriod));
  const useEma = settings.maType !== 'SMA';
  const stopLossTicks = Math.max(1, Number(settings.stopLossTicks));
  const targetRR = Math.max(0.5, Number(settings.targetRR));
  const maxTradesPerDay = Math.max(1, Number(settings.maxTradesPerDay));
  const maxHoldCandles = Math.max(1, Number(settings.maxHoldCandles));
  ...
  const trades = [];
  for (const [, dayCandles] of byDay.entries()) {
    if (dayCandles.length < slowPeriod + 2) continue;

    const closes = dayCandles.map((c) => Number(c[4]));
    const highs = dayCandles.map((c) => Number(c[2]));
    const lows = dayCandles.map((c) => Number(c[3]));
    const fastMa = useEma ? calculateEma(closes, fastPeriod) : calculateSma(closes, fastPeriod);
    const slowMa = useEma ? calculateEma(closes, slowPeriod) : calculateSma(closes, slowPeriod);
    ...
    for (let i = 1; i < dayCandles.length && tradesToday < maxTradesPerDay; i += 1) {
      ...
      const bullishCross = fastMa[i] > slowMa[i] && fastMa[i - 1] <= slowMa[i - 1];
      const bearishCross = fastMa[i] < slowMa[i] && fastMa[i - 1] >= slowMa[i - 1];
      if (!bullishCross && !bearishCross) continue;

      const side = bullishCross ? 'LONG' : 'SHORT';
      const entryPrice = closes[i];
      if (side === 'LONG') {
        const stopLoss = entryPrice - stopLossTicks;
        const target = entryPrice + stopLossTicks * targetRR;
        position = { side, entryPrice, stopLoss, target, entryIndex: i, entryTime: dayCandles[i][0] };
      } else {
        const stopLoss = entryPrice + stopLossTicks;
        const target = entryPrice - stopLossTicks * targetRR;
        position = { side, entryPrice, stopLoss, target, entryIndex: i, entryTime: dayCandles[i][0] };
...
app.post('/api/strategy3/run', async (req, res) => {
  try {
    const {
      symbol = 'BANKNIFTY',
      interval = '5',
    ...
    const result = runStrategyThree({
      candles: payload.rows,
      settings: {
        symbol: String(symbol).toUpperCase(),
        qty: Number(qty),
        maType: String(maType).toUpperCase() === 'SMA' ? 'SMA' : 'EMA',
        fastMaPeriod: Number(fastMaPeriod),
        slowMaPeriod: Number(slowMaPeriod),
    ...
    return res.json({
      ok: true,
      runId: runDoc._id,
      strategy: 'Strategy 3 - Moving Average Crossover',
      year: Number(year),
      symbol: String(symbol).toUpperCase(),
      interval: String(interval),
      summary: result.summary,
...
app.get('/api/strategy3/runs/:runId/trades', async (req, res) => {
  try {
    const { runId } = req.params;
    const page = Math.max(1, Number(req.query.page) || 1);
    const pageSize = Math.min(500, Math.max(10, Number(req.query.pageSize) || 25));
